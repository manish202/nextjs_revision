...
import {userActionForServerCompo} from "@/app/server_actions_crud/revalidatepath_test/user.action";
import ClientComponent from "@/components/server_actions_crud/revalidatepath_test/ClientComponent";
...
const getUsers = async () => {
    try{
        await connectDB();
        const users = await User.find().sort({updatedAt:-1}).skip(0).limit(5);
                    ...
                    <div className="mb-6 flex items-center justify-between">
                        <div className="w-5/6">
                            ...
                            <p className="mt-2 text-sm text-gray-500">
                                Here i am testing revalidatePath() method (clear the cache and trigger a new request to the server.)
                                click the right button, it will call server action/function who is using revalidatePath() method.
                                Its for server component.
                            </p>
                        </div>
                        <form action={userActionForServerCompo}>
                            <button type="submit" className="w-1/6 rounded-md bg-blue-600 px-5 py-2.5 text-sm font-medium text-white transition hover:bg-blue-700">
                                Generate random user and revalidatePath
                            </button>
                        </form>
                    </div>
